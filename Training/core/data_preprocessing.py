...
def data_preprocessing(data_type='base', feature_type='drebin', is_training_set=True, is_finetune=False):
    ...
    if not is_finetune:
        save_path = os.path.join(intermediate_data_saving_dir,
                             'databases_' + feature_type + '_' + str(data_type) + '.conf')
    else:
        save_path = os.path.join(intermediate_data_saving_dir,
                                 'databases_finetune_' + feature_type + '_' + str(data_type) + '.conf')
    ...
    feature_extractor = feature_type_scope_dict[feature_type](android_features_saving_dir,
                                                              ...
                                                              update=False,
                                                              proc_number=8)
    oos_features = [os.path.join(android_features_saving_dir, filename) + '.' + feature_type for filename in
                    data_filenames]
    if is_training_set:
        dataset, input_dim, dataX_np = feature_extractor.feature2ipt(oos_features, gt_labels)
    else:
